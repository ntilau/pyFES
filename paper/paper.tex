...
A single FEM solve (including waveguide port assembly) requires approximately 55~ms on an Apple M3 MacBook Air. Generating the full 4050-sample training set takes approximately 3.7~minutes. After training, the MoE-DKL surrogate evaluates 4050 points in 356~ms---a speedup factor of approximately $6\times 10^2$ over the FEM solver. Training all four models completes in approximately 30~minutes on the same hardware (MPS acceleration).
