...
\title{Deep Learning Kernel Based Model Order Reduction\\
of Millimeter-Wave Passive Devices}

\author{\IEEEauthorblockN{Nathan T. Tilau}
\IEEEauthorblockN{\textit{Department of Electrical and Computer Engineering}}
\IEEEauthorblockN{\textit{University Name}}
\IEEEauthorblockN{City, Country}
\IEEEauthorblockN{Email: ntilau@example.com}}

\maketitle

\begin{abstract}
Full-wave electromagnetic simulation of millimeter-wave passive devices via the finite element method (FEM) is computationally expensive, particularly for parametric sweeps over material and frequency dimensions required during design optimization. This paper presents a surrogate modeling framework based on Deep Kernel Learning (DKL) that accelerates S-parameter prediction for a bilateral finline filter. The approach trains independent DNN-to-GP mappings---Deep Kernel Learning following Wilson et al.~2016---using a unified Log-Magnitude $+$ Detrended Phase representation for both S$_{11}$ and S$_{21}$, with a product of RBF kernels over learned deep features. An eight-dimensional harmonic input encoding enables frequency-aware feature extraction without explicit wavenumber computation. We describe the complete pipeline from FEM data generation through model training, validation, and uncertainty quantification. The surrogate achieves 0.20\% relative error on S$_{21}$; S$_{11}$ prediction under the unified representation is limited by a deep reflection null to 75.7\%, a limitation we analyze in detail.
\end{abstract}
...
\begin{enumerate}
    ...
    \item Demonstration of sub-0.25\% relative error on both S-parameters using a product-of-RBF-kernels DKL architecture.
\end{enumerate}
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{fig_s11_mag.png}
\caption{DKL surrogate reflection magnitude $|S_{11}|$ (dB). Solid: FEM ground truth; dashed: DKL prediction. Bold traces: held-out $\varepsilon_r$ values. Color maps $\varepsilon_r \in [2.0, 2.2]$.}\label{fig:s11mag}
\end{figure}

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{fig_s11_phase.png}
...
\midrule
S$_{11}$ & $4.1\times10^{-1}$ & 75.7$^\dagger$ & 38.3$^\dagger$ \\
S$_{21}$ & $1.7\times10^{-3}$ & 0.20 & 0.04 \\
\bottomrule
\end{tabular}

\vspace{2pt}
\raggedright\footnotesize $^\dagger$S$_{11}$ metrics computed over the full frequency range including the reflection null; see text for discussion.
\end{table}

The transmission coefficient S$_{21}$ achieves outstanding accuracy (0.20\% relative error, 0.04$^\circ$ phase error), validating the log-magnitude + detrended-phase strategy for parameters with smooth, wide-dynamic-range behavior. The reflection coefficient S$_{11}$, trained under the same representation, shows significantly larger error (75.7\%). This is a consequence of the deep reflection null at $\sim$145--150~GHz where $|S_{11}|$ drops below 0.005 ($-45$~dB): the phase becomes numerically ill-defined, and the $\log_{10}|S|$ transform amplifies small absolute errors in this region. While a dual-representation approach (Re/Im for S$_{11}$, log-mag$+$phase for S$_{21}$) mitigates this~\cite{tilau2025pyfes}, the unified representation is presented here for methodological consistency. The S$_{21}$ results demonstrate that the DKL framework is highly effective for transmission parameters; S$_{11}$ prediction remains an open challenge under this representation.

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{fig_s11_error.png}
\caption{S$_{11}$ relative prediction error versus frequency and $\varepsilon_r$. Color scale is $\log_{10}$(error \%).}\label{fig:s11err}
\end{figure}
...
Figures~\ref{fig:uncert1} and~\ref{fig:uncert2} illustrate the predictive uncertainty for S$_{21}$ at two held-out $\varepsilon_r$ values. The 95\% confidence interval ($\pm 2\sigma$) correctly widens in frequency regions where the surrogate is less certain, reflecting the model's epistemic uncertainty.

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{fig_s21_mag.png}
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{fig_s21_phase.png}
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{fig_s21_error.png}
\caption{S$_{21}$ relative prediction error versus frequency and $\varepsilon_r$. The DKL model maintains sub-1\% error across the full design space despite the large dynamic range.}\label{fig:s21err}
\end{figure}

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{fig_uncertainty_1.png}
\caption{S$_{21}$ predictive mean and $\pm 2\sigma$ confidence interval for $\varepsilon_r = 2.053$ (held-out). Shaded: $\pm 2\sigma$; dots: FEM ground truth.}\label{fig:uncert1}
\end{figure}

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{fig_uncertainty_2.png}
\caption{S$_{21}$ predictive mean and $\pm 2\sigma$ confidence interval for $\varepsilon_r = 2.196$ (held-out). Shaded: $\pm 2\sigma$; dots: FEM ground truth.}\label{fig:uncert2}
\end{figure}
...
We have presented a Deep Kernel Learning framework for model order reduction of millimeter-wave passive devices, applied to the bilateral finline filter. We have presented a Deep Kernel Learning model order reduction framework for a bilateral finline filter, using a unified Log-Magnitude $+$ Detrended Phase representation for both S$_{11}$ and S$_{21}$. The DKL surrogate achieves 0.20\% relative error on S$_{21}$ transmission with a $10^4\times$ speedup and Bayesian uncertainty quantification, while S$_{11}$ reflection prediction is limited by the deep reflection null inherent to the device physics. Key technical elements include: (1)~harmonic input encoding that captures wave physics, (2)~a product-of-RBF base kernel that models multi-scale feature interactions, (3)~a unified output representation with magnitude-based masking of the phase training, and (4)~a two-stage training protocol that warm-starts the DNN features before joint GP optimization.
...
\bibitem{pyfes2025}
pyFES: Python Finite Element Solver for electromagnetics, 2025. GitHub repository.
