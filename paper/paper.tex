...
\title{Mixture of Experts Deep Kernel Learning for\\
Millimeter-Wave Passive Devices Model Order Reduction}
...
The FEM solver achieves approximately 55~ms per frequency solve (including waveguide port assembly) on an Apple M3 MacBook Air, yielding a total dataset generation time of 3.7~minutes for 4050 samples. After training, the MoE-DKL surrogate evaluates 4050 points in 356~ms---a batch speedup factor of approximately $6\times 10^2$ over the FEM solver. The per-point inference cost is 88~$\mu$s with an amortized Cholesky decomposition. Training all four MoE-DKL models (500 epochs each) completes in approximately 30~minutes on the same hardware using Apple MPS acceleration. The total workflow (data generation + training) thus requires under 35~minutes, after which the surrogate provides near-instantaneous S-parameter predictions with full uncertainty quantification.
