...
The complex S-parameters have fundamentally different physical characteristics that demand distinct output representations.

\subsubsection{S$_{11}$ (Reflection): Direct Re/Im}
The reflection coefficient $S_{11}$ undergoes a deep null at the filter's passband ($|S_{11}| < 0.005$, below $-45$~dB) where the bilaterally symmetric finline provides near-perfect matching. At this null, the phase wraps rapidly and becomes numerically ill-defined --- a log-magnitude $+$ phase representation would amplify this noise and fail to converge. Instead, we regress directly on the real and imaginary parts:
\begin{align}
...
The transmission coefficient spans a large dynamic range: $|S_{21}| \approx 1.0$ in the 144--150~GHz passband, dropping to $\sim 0.07$ in the notch near 140--143~GHz. A 14$\times$ amplitude range makes raw Re/Im regression inefficient, as the solver would allocate disproportionate capacity to the passband. We instead predict
\begin{align}
...
\end{align}
where the linear phase trend ${\alpha_{\varepsilon_r} f + \beta_{\varepsilon_r}}$ is fitted per $\varepsilon_r$ via least squares~\cite{kay1998fundamentals}. The detrended phase isolates the frequency-dependent fine structure from the smooth group delay.

During inference, S$_{11}$ is reconstructed directly as $\hat{y}_1 + j\hat{y}_2$, while S$_{21}$ is reconstructed via $|S_{21}| = 10^{\hat{y}_3}$ with the phase restored as $\angle S_{21}^{\text{(pred)}} = \hat{y}_4 + \alpha_{\varepsilon_r} f + \beta_{\varepsilon_r}$.
